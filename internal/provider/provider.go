			...
			"git": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					...
					"read_only": schema.BoolAttribute{
						Description: "If true, Flux will not commit/push any git repo changes. Defaults to false",
						Optional:    true,
					},
				},
				Optional: true,
...
func (p *fluxProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if data.Git.ReadOnly.IsNull() {
		data.Git.ReadOnly = types.BoolValue(false)
	}
